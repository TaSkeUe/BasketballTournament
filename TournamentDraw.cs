public static class TournamentDraw{
    ...
    public static List<(Teams, Teams)> DrawQuarterfinals(Dictionary<string, List<Teams>> seeds, Dictionary<string, List<MatchResult>> matchResults)
    {
        ...
        var quarterfinals = new List<(Teams, Teams)>();
        ...
        foreach (var teamD in potD)
        {
            ...
            {
                // Ako ne može da pronađe odgovarajućeg protivnika, dodaj prvi dostupan
                opponent = potG.First();
                quarterfinals.Add((teamD, opponent));
                potG.Remove(opponent);
            }
        ...
        // Provera da li je sve dobro raspoređeno
        if (potG.Count > 0 || potF.Count > 0)
        {
            Console.WriteLine("Warning: Some teams were not properly assigned to quarterfinals.");
        }
